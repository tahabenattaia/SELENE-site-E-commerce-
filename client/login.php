...
{
 $clients=login($_POST);
if (!empty($clients) AND $clients==true){
  session_start();
  $_SESSION['email']=$clients['email'];
  $_SESSION['id']=$clients['id'];
  $_SESSION['nom']=$clients['nom'];
  $_SESSION['prenom']=$clients['prenom'];
  $_SESSION['telephone']=$clients['telephone'];
  $_SESSION['adresse']=$clients['adresse'];
  $_SESSION['ps']=$clients['ps'];
  $_SESSION['etat']=$clients['etat'];
  header("location:profil.php");
}
}
?>
...
Swal.fire({
  icon: 'error',
  title: 'Oops...',
  text: 'Ce compte n existe pas , Veuillez register',
})
</script>");
}
?>
